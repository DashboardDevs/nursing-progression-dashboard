            ...
            return (
                <div className="mt-8">
                    <div className="flex items-center ml-5 mr-5">
                        <h1 className="bg-gray-200 w-2/6 text-center font-semibold py-1 rounded-lg text-4xl">{this.state.student.last_name}, {this.state.student.first_name}</h1>
                        <div className=" bg-gray-200 mx-2 h-4 w-full rounded-full" >
                            <div className="bg-green-500 h-full rounded-full" style={{ width: setWidth }}></div>
                        </div>
                    </div>
                    <div className="flex">
                        <p id="gradDate" className="bg-gray-200 w-2/6 text-left font-semibold py-1 rounded-lg text-xl ml-5 mt-3">Expected Graduation Date: {this.state.student.graduation_date.slice(5, 7)}/{this.state.student.graduation_date.slice(8, 10)}/{this.state.student.graduation_date.slice(0, 4)}</p>
                        <div className="w-4/6">
                            <Link className="float-right bg-scarlet text-white py-2 px-6 rounded-3xl h-2/3 mt-5 mr-6" to={{ pathname: "/request", student: this.state.student }}>Update Milestones</Link>
                        </div>
                    </div>
                    <div className="ml-5 mr-5 mt-3 p-5 bg-gray-200 rounded-lg">
                                ...
                                return (
                                    <div key={milestone.id}>
                                        <CollapsibleHead class="pb-5 pl-5 bg-gray-400 rounded-lg">
                                            ...
                                            <div className="w-10/12 float-right">{milestone.name}</div>
                                        </CollapsibleHead>
            ...
            return (
                <div className="mt-8">
                    ...
                    <div className="flex">
                        ...
                        <Link className="float-right bg-scarlet text-white py-2 px-6 rounded-3xl h-2/3 mt-5 mr-6" to={{ pathname: `/advisor/${this.props.currentUser.id}`}}>Back</Link>
                            <a className="float-right bg-gray-200 textgray-400 py-2 px-6 rounded-3xl h-2/3 mt-5 mr-6" href="https://login.microsoftonline.com/?whr=buckeyemailosu.onmicrosoft.com" rel="noreferrer" target="_blank">OneDrive</a>
                            <Link className="float-right bg-scarlet text-white py-2 px-6 rounded-3xl h-2/3 mt-5 mr-6" to={{ pathname: "/update", student: this.state.student }}>Update Milestones</Link>
                        </div>
                    </div>
                    <div className="ml-5 mr-5 mt-3 p-5 bg-gray-200 rounded-lg">
                        <CollapsibleComponent class="m-3  p-5 text-black font-bold text-center text-base bg-white grid grid-cols-4 gap-3">
